  ...
  const connectWallet = useCallback(async () => {
    ...
    try {
      ...
      if (Number(network.chainId) !== targetChainId) {
        toast.warn(`Please switch to Sepolia Testnet. Current network: ${network.name}`);
        try {
      ...
      toast.success(`Connected to ${address.slice(0, 6)}...${address.slice(-4)} on ${network.name || 'Network'}`);
    } catch (error: any) {
      console.error('Wallet connection failed:', error);
      if (error.code === 4001) {
        toast.error('Wallet connection rejected by user');
      } else if (error.code === -32002) {
        toast.error('MetaMask connection request is pending. Please check MetaMask.');
      } else {
